...
const bmp_header_parts = [
    ...
    // 24-bit: width * height * bytes_per_pixel + (height * padding) + 54
    // 32-bit: width * height * bytes_per_pixel + 54
    //
...
function bmp_get_pixel(resource, x, y) {
    // NOTE(oxou): We need this or we get pixels drawn on the opposite side
    x = Math.floor(x);
    y = Math.floor(y);
 ...
 * @return         false | HTMLImageElement | HTMLCanvasElement
 */
